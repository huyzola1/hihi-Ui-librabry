...
function UILib:CreateWindow(config)
	...
	local Main = Create("Frame", {
		...
		Position = UDim2.new(0.5, 0, 0.5, 0), Size = fullSize,
		ClipsDescendants = true, BackgroundTransparency = 1, Parent = screenGui,
	}, { Corner(12) })

	local TopBar = Create("Frame", { Name = "TopBar", BackgroundColor3 = Theme.Surface, Size = UDim2.new(1, 0, 0, 48), Parent = Main }, { Corner(12) })
	...
	local ToggleButton = Create("ImageButton", { Name = "ToggleButton", BackgroundColor3 = Theme.Accent, AnchorPoint = Vector2.new(0.5, 0.5), Position = UDim2.new(0, 45, 0.5, 0), Size = UDim2.new(0, 0, 0, 0), Visible = false, AutoButtonColor = false, Parent = screenGui }, { Corner(25) })
	Create("ImageLabel", { BackgroundTransparency = 1, Position = UDim2.new(0.5, -12, 0.5, -12), Size = UDim2.new(0, 24, 0, 24), Image = "rbxassetid://6031094670", ImageColor3 = Color3.fromRGB(255, 255, 255), Parent = ToggleButton })
	...
	task.delay(0.1, function()
		isAnimating = true; Main.ClipsDescendants = true
		Main.Size = UDim2.new(0, 0, 0, 0); Main.BackgroundTransparency = 1
	...
	function Window:CreateTab(tabName, iconId)
		local currentIcon = iconId or "rbxassetid://6034818372"
		...
		local TabIcon = Create("ImageLabel", { BackgroundTransparency = 1, Position = UDim2.new(0, 0, 0.5, -8), Size = UDim2.new(0, 16, 0, 16), Image = currentIcon, ImageColor3 = Theme.SubText, Parent = TabButton })
		local TabText = Create("TextLabel", { BackgroundTransparency = 1, Position = UDim2.new(0, 24, 0, 0), Size = UDim2.new(1, -34, 1, 0), Font = Theme.Font, Text = tabName, TextColor3 = Theme.SubText, TextSize = 13, TextXAlignment = Enum.TextXAlignment.Left, TextTruncate = Enum.TextTruncate.AtEnd, Parent = TabButton })
		...
		function Tab:CreateButton(opts)
			opts = opts or {}
			local Btn = Create("TextButton", { BackgroundColor3 = Theme.Surface, Size = UDim2.new(1, 0, 0, 38), Font = Theme.Font, Text = opts.Name or "Button", TextColor3 = Theme.Text, TextSize = 13, AutoButtonColor = false, Parent = TabPage }, { Corner(8) })
	...
	function Window:Notify(opts)
		...
		local Notif = Create("Frame", { BackgroundColor3 = Theme.Surface, Size = UDim2.new(1, 0, 0, 0), AutomaticSize = Enum.AutomaticSize.Y, LayoutOrder = -os.clock(), Position = UDim2.new(1, 320, 0, 0), Parent = notifHolder }, { Corner(10) })
		Create("Frame", { BackgroundColor3 = Theme.Accent, Size = UDim2.new(0, 4, 1, -8), Position = UDim2.new(0, 0, 0, 4), Parent = Notif }, { Corner(4) })
		Create("TextLabel", { BackgroundTransparency = 1, Position = UDim2.new(0, 16, 0, 8), Size = UDim2.new(1, -28, 0, 18), Font = Theme.FontBold, Text = opts.Title or "Thông báo", TextColor3 = Theme.Text, TextSize = 14, TextXAlignment = Enum.TextXAlignment.Left, Parent = Notif })
		...
		Create("UIPadding", { PaddingBottom = UDim.new(0, 10) }).Parent = Notif
		Tween(Notif, { Position = UDim2.new(0, 0, 0, 0) }, 0.4, Enum.EasingStyle.Quad)
		task.delay(duration, function()
			if Notif and Notif.Parent then
				Tween(Notif, { Position = UDim2.new(1, 320, 0, 0), BackgroundTransparency = 1 }, 0.3)
